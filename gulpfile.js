...
//********
var sMyAppName = "ZODATA_INIT2/WebContent"; //****Change it to your app name before you run gulpcmd.bat
//********
...
gulp.task('streamifyViewXML', ['streamifyComponentJS'], function () {
	...
	return gulp.src('./WebContent/view/*.xml')
	.pipe(prettyData({
			type : 'minify',
...
gulp.task('streamifyViewJS', ['streamifyViewXML'], function () {
	...
	return gulp.src('./WebContent/view/*.js')
	.pipe(uglify())
	.pipe(tap(function (file, t) {
...
gulp.task('streamifyUtilsJS', ['streamifyViewJS'], function () {
	...
	return gulp.src('./WebContent/utils/*.js')
	.pipe(uglify())
	.pipe(tap(function (file, t) {
...
gulp.task('streamifyModelJSON', ['streamifyUtilsJS'], function () {
	...
	return gulp.src('./WebContent/model/*.json')
	.pipe(prettyData({
			type : 'minify',
			preserveComments : false
		}))
...
gulp.task('streamifyModelXML', ['streamifyModelJSON'], function () {
	...
	return gulp.src('./WebContent/model/*.xml')
	.pipe(prettyData({
			type : 'minify',
			preserveComments : false
		}))
...
gulp.task('myWriteToFile', ['streamifyModelXML'], function () {
	...
	var sFileNameQ = "./WebContent/" + sFilename + ".js"; //Component-preload.js;
